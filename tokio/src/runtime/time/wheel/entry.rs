...
impl Handle {
    ...
    pub(crate) fn wake(&self) {
        ...
        match *lock {
            ...
            // don't unlock — poisoning the `Mutex` stops others from using the bad state.
            State::Registered(_) => panic!("corrupted state: State::Registered"),
            // don't unlock — poisoning the `Mutex` stops others from using the bad state.
    ...
    pub(crate) fn transition_to_registered(&self, cancel_tx: Sender) -> TransitionToRegistered {
        ...
        match *lock {
            ...
            // don't unlock — poisoning the `Mutex` stops others from using the bad state.
            State::Registered(_) => panic!("should not be called twice"),
            // don't unlock — poisoning the `Mutex` stops others from using the bad state.
            State::Pending => panic!("entry is already pending, cannot register again"),
            // don't unlock — poisoning the `Mutex` stops others from using the bad state.
            State::WokenUp => panic!("already woken up, cannot register again"),
    ...
    pub(crate) fn transition_to_cancelling(&self) {
        ...
        match *lock {
            ...
            // no need to cancel pending entry
            State::Pending => *lock = State::Cancelling,
            // no need to cancel woken up entry
            State::WokenUp => *lock = State::Cancelling,
            // don't unlock — poisoning the `Mutex` stops others from using the bad state.
